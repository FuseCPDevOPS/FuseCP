...
# Rewrites exact alert spans for:
...
$targets = $alerts | Where-Object {
    ($_.rule.id -eq 'js/trivial-conditional' -or $_.rule.id -eq 'js/unneeded-defensive-code') -and
    $_.most_recent_instance.location.path -like 'FuseCP/Sources/*.js' -and
    $_.most_recent_instance.location.path -notlike '*/node_modules/*'
}

Write-Host "JS_CONST_TARGETS=$($targets.Count)"
...
foreach ($a in $targets) {
    ...
    $byFile[$loc.path].Add([pscustomobject]@{
        Rule = $a.rule.id
        Message = [string]$a.most_recent_instance.message.text
...
function Get-ReplacementFromMessage([string]$msg) {
    if ($msg -match 'always evaluates to true') { return 'true' }
    if ($msg -match 'always evaluates to false') { return 'false' }
    if ($msg -match 'always evaluates to true\.') { return 'true' }
    if ($msg -match 'always evaluates to false\.') { return 'false' }
    return $null
}
...
foreach ($entry in $byFile.GetEnumerator()) {
    ...
    foreach ($it in $alertsInFile) {
        $replacement = Get-ReplacementFromMessage $it.Message
        if ($null -eq $replacement) { $totalSkipped++; continue }
        ...
        if ([string]::IsNullOrWhiteSpace($span)) { $totalSkipped++; continue }

        if ($span -eq 'true' -or $span -eq 'false') { $totalSkipped++; continue }
...
Write-Host "JS_CONST_FIXED=$totalFixed"
Write-Host "JS_CONST_FILES_CHANGED=$filesChanged"
Write-Host "JS_CONST_SKIPPED=$totalSkipped"