...
namespace FuseCP.EnterpriseServer
{
    public class SuspendOverusedPackagesTask : SchedulerTask
    {
        ...
        public override void DoWork()
        {
            ...
            foreach (PackageInfo package in packages)
            {
                try
                {
                    ...
                    {
                        SendEmail(warningMailFrom, userInfo.Email, warningMailBcc, warningMailSubjectProcessed, warningMailBodyProcessed, false);
                    }

                    if (sendSuspensionEmail && (!isDiskSpaceBelowSuspensionThreshold || !isBandwidthBelowSuspensionThreshold))
                    {
                        SendEmail(suspensionMailFrom, userInfo.Email, suspensionMailBcc, suspensionMailSubjectProcessed, suspensionMailBodyProcessed, false);
                    }
                    ...
                    if (suspendOverused && (!isDiskSpaceBelowSuspensionThreshold || !isBandwidthBelowSuspensionThreshold))
                    {
                        ...
                        TaskManager.Write(String.Format("Suspend space '{0}' of user '{1}'", package.PackageName, user.Username));
                ...
                {
                    failedPackages++;
                    TaskManager.WriteError("SuspendOverusedPackagesTask failed for package '{0}'. Error: {1}", package.PackageName, ex.ToString());
                }
            }

            TaskManager.Write("Total packages suspended: " + suspendedPackages);
            if (failedSuspensions > 0)
            {
                TaskManager.WriteWarning("Total packages failed to suspend: " + failedSuspensions);
            }
            if (failedPackages > 0)
            {
                TaskManager.WriteWarning("Total packages failed to process: " + failedPackages);
            }
        }
        ...
        private void SendEmail(string from, string to, string bcc, string subject, string body, bool isHtml)
        {
            ...
            {
                TaskManager.WriteError("SuspendOverusedPackagesTask e-mail failed for recipient '{0}'. Error: {1}", to, ex.ToString());
            }
        }
    }
}
