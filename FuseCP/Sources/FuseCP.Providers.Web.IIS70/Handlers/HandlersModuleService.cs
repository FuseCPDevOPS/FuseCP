	...
	internal sealed class HandlersModuleService : ConfigurationModuleService
	{
			...
			IEnumerable<string> extensions, string processor, string scName, string scModule)
		{
			...
			using (var srvman = GetServerManager())
			{
				...
				foreach (var extParts in extensions.Where(extension => FindHandlerAction(handlersCollection, extension.Split(',')[0], processor) == null).Select(extension => extension.Split(',')))
				{
					var path = extParts[0];
